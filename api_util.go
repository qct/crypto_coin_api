...
func CancelAllUnfinishedOrders(api Api, cp CurrencyPairV2) int {
    if api == nil {
    ...
    if orders != nil {
        c := 0
        for _, ord := range orders.([]OrderV2) {
...
func CancelAllUnfinishedFutureOrders(api FutureApi, contractType string, cp CurrencyPairV2) {
    if api == nil {
        log.Println("api instance is nil, please new a api instance")
        return
    }
